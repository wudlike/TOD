...
def tod(channels, dnu, band):
    ...
    Tod = sum(np.multiply(
        np.mat(channels[1]).T*dnu, np.mat(map_combine_smoothed)))
#    print(Tod)
...
def conv(channels, dnu, band):
    ...
    """
    map_I_smoothed = [map_smoothing(nu, dust_map(
        nu, dust_ind1, dust_ampl_I), FWHM(nu)) for nu in band]
    I_conv = sum(np.multiply(
        np.mat(channels[1]).T*dnu, np.mat(map_I_smoothed)))
    map_Q_smoothed = [map_smoothing(nu, dust_map(
        nu, dust_ind1, dust_ampl_I), FWHM(nu)) for nu in band]
    Q_conv = sum(np.multiply(
        np.mat(channels[1]).T*dnu, np.mat(map_Q_smoothed)))
    map_U_smoothed = [map_smoothing(nu, dust_map(
        nu, dust_ind1, dust_ampl_I), FWHM(nu)) for nu in band]
    U_conv = sum(np.multiply(
        np.mat(channels[1]).T*dnu, np.mat(map_U_smoothed)))
...
hp.write_map('tod_sc.fits', tod, extra_header=(
    [('name', 'TOD'), ('nside', '2048')]), overwrite=True)
